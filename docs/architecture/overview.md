...
PostgreSQL is the only database. Models and migrations live under `apps/backend`. Development runs services in Compose, while Expo runs on the host so developers can use native simulators/devices. Optional MinIO and Mailpit live behind Compose profiles. No queue is installed until a real product requires one.
